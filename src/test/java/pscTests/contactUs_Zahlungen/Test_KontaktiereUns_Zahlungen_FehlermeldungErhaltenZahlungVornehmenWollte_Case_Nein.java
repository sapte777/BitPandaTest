...
public class Test_KontaktiereUns_Zahlungen_FehlermeldungErhaltenZahlungVornehmenWollte_Case_Nein extends BaseClass{
    ...
    private String seriennummer =CRTestUtils.getRandomNumberString(9);

    @Test(description = "Contact us login fill form" , retryAnalyzer = RetryAnalyzer.class)
    public void contactUs_Zahlungen_FehlermeldungErhaltenZahlungVornehmenWollte_Nein() {
...
//      Step 3: Select "Ich habe eine Fehlermeldung erhalten, als ich eine Zahlung vornehmen wollte" option from the
//      dropdown menu on "What is your enquiry about?" topic
        KontaktiereUns.clickFehlermeldungErhaltenWennZahlungVornehmenWollte();
        KontaktiereUns.clickNextStepContactUs();
        bereitsPSCKontoText=KontaktiereUns.getBereitsPSCKontoText();
        Assert.assertEquals(bereitsPSCKontoText,bereitsPSCKontoTitle);

//      Step 4: Select "Ja" option from "Ich habe eine Fehlermeldung erhalten, als ich eine Zahlung vornehmen wollte"
//      topic
        KontaktiereUns.clickJuniorPIN_Nein();
        KontaktiereUns.clickNextStepContactUs();
...
//      Step 4: Upload document file and insert email address on "Required information" topic
        KontaktiereUns.insertSerieNumberZahlungen(seriennummer);
